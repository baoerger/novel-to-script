...
支持 `.txt` / `.docx` 输入，自动分章、提取角色与场景、汇总去重、生成结构化 YAML 剧本，并提供 Web 前端进行可视化交互。

> **演示视频**：[【小说转剧本 demo】](https://www.bilibili.com/video/BV1fqEt6xEbV/)
>
> 完整流程演示：小说上传 → 实时进度 → YAML 在线预览 → 角色表格 → 剧本编辑。

---

## 开发过程

这条流水线是在 **2026-06-05 → 06-07 三天**内从零建起来的，共 **116 次提交、46 个 PR**。
提交历史按真实增量保留，可以直接看出工程推进的顺序：

| 阶段 | 内容 |
| --- | --- |
| 第 1 天 | 设计文档、实施计划、FastAPI 骨架、配置模块 |
| 第 2 天 | 数据模型 → 6 个 Prompt 模板 → 解析器 / 分片器 → LLM 抽象层 → 提取与汇总服务 → 流水线编排 → API 与 CLI |
| 第 3 天 | Mock LLM 单元测试、Dockerfile / compose、断点续跑与并行化、质检 Agent、质量报告、React 前端与全栈 Docker 集成 |

推进节奏是「一个功能 → 一个 PR → 合并 → 更新进度追踪」，因此 `progress.md`
里能查到每一步对应的 PR 编号。第 3 天的测试与容器化是在功能全部跑通之后补的；
晚间的重构提交（导入路径统一、解析器鲁棒性增强）也是真实留下的返工记录，
不是一次成型的代码。
...
以下为本项目自主设计与实现的核心模块，不依赖任何第三方 AI 转换引擎或 SaaS 服务：

| 模块 | 原创内容说明 |
|------|-------------|
| **转换流程引擎** | `pipeline.py` — 从小说解析到剧本生成的 7 阶段完整流水线（分片→逐章分析→跨章汇总→关系提取→场景生成→组装→输出），包含断点续跑机制 |
| **AI Prompt 体系** | `prompts/` 目录下 6 个 Jinja2 模板 — 角色提取、场景检测、对白提取、跨章汇总、关系提取、场景生成，所有 Prompt 均针对中文小说特点独立设计调优 |
...
本项目所调用的大语言模型能力（OpenAI 兼容 API）为通用 AI 基础设施，不属于特定第三方产品。
项目未使用任何现成的「小说转剧本」商业 SDK 或 SaaS 平台。

本项目为个人独立完成的课程项目，全部模块与文档由本人编写。
